...
describe('OktaDev Schematics: Angular', () => {
  ...
  it('should add routes for callback', (done) => {
    schematicRunner.runSchematicAsync('add-auth', defaultOptions, appTree).toPromise().then(tree => {
      const routingModule = tree.readContent('/projects/authtest/src/app/oidc-routing.module.ts');
  ...
  it('should import the oidc-routing module in the app module file', (done) => {
    schematicRunner.runSchematicAsync('add-auth', defaultOptions, appTree).toPromise().then(tree => {
      const appModule = tree.readContent('/projects/authtest/src/app/app.module.ts');
      expect(appModule).toMatch(/OidcRoutingModule/);
      done();
    }, done.fail);
  });

  it('should set the issuer & clientId in the oidc-routing module file', (done) => {
    schematicRunner.runSchematicAsync('add-auth', defaultOptions, appTree).toPromise().then(tree => {
      const appModule = tree.readContent('/projects/authtest/src/app/oidc-routing.module.ts');
